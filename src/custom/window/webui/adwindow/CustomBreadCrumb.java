...
public class CustomBreadCrumb extends Div implements EventListener<Event> {
	...
	/** Last DataStatusEvent from {@link CustomAbstractADWindowContent#dataStatusChanged(DataStatusEvent)} **/
	private DataStatusEvent m_dse;

	/** Last data status text from {@link CustomAbstractADWindowContent#dataStatusChanged(DataStatusEvent)} **/
	...
	private CustomAbstractADWindowContent windowContent;

	/**
	 * @param windowContent
	 * @param windowNo
	 */
	public CustomBreadCrumb(CustomAbstractADWindowContent windowContent, int windowNo) {
	...
	public void addLinks(LinkedHashMap<String, String> links) {
		...
		EventListener<Event> listener = new EventListener<Event>() {
			...
			private void showLinksMenu(final Label pathLabel) {
				for(Map.Entry<String, String>entry : CustomBreadCrumb.this.links.entrySet()) {
					final Menuitem item = new Menuitem();
					item.setLabel(entry.getValue());
					item.setValue(entry.getKey());
					item.addEventListener(Events.ON_CLICK, CustomBreadCrumb.this);
	...
	public void onEvent(Event event) throws Exception {
		if (event.getTarget() == btnRecordInfo) {
			...
			winfo.addCallback(Window.AFTER_PAGE_DETACHED, t -> {
				CustomADWindow adwindow = CustomADWindow.findADWindow(CustomBreadCrumb.this);
				if (adwindow != null)
    ...
    /**
     * Data status from {@link CustomAbstractADWindowContent#dataStatusChanged(DataStatusEvent)}
