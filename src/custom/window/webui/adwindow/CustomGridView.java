 ...
 *
 */
public class CustomGridView extends Vlayout implements EventListener<Event>, IdSpace, IFieldEditorContainer, StateChangeListener
{
	...
	/**	Static Logger	*/
	private static CLogger	s_log	= CLogger.getCLogger (CustomGridView.class);
	...
	private CustomGridTabRowRenderer renderer;
	 ...
	 * @param windowNo
	 */
	public CustomGridView(int windowNo)
	{
	...
	/**
	 * Update {@link CustomDetailPane} status with record count
	 */
	private void showRecordsCount() {
		Component parent = this.getParent();
		while (parent != null) {
			if (parent instanceof CustomDetailPane) {
				CustomDetailPane p = (CustomDetailPane) parent;
	...
	private void updateModel() {
		...
		if (renderer != null && renderer.isEditing())
			renderer.stopEditing(false);
		renderer = new CustomGridTabRowRenderer(gridTab, windowNo);
			...
			if (data != null && data instanceof Component)
			{
				...
				else
				{
					...
					if (cmp.getParent() instanceof org.zkoss.zul.Row)
					{
						...
						columnName = (String) cmp.getAttribute(CustomGridTabRowRenderer.COLUMN_NAME_ATTR);
					}
				}
			}
			if (row != null)
...
//				}
	...
	private void focusToFirstEditorIfNotDetailTab() {
		CustomADTabpanel adtabpanel = null;
		boolean setFocus = true;
		Component parent = listbox.getParent();
		while (parent != null) {
			if (parent instanceof CustomADTabpanel) {
				adtabpanel = (CustomADTabpanel) parent;
				break;
			}
			parent = parent.getParent();
		}
		if (adtabpanel != null)
		{
			CustomADWindow adwindow = CustomADWindow.findADWindow(adtabpanel);
			if (adwindow != null) {
				CustomIADTabpanel selectedADTabpanel = adwindow.getADWindowContent().getADTab().getSelectedTabpanel();
				CustomIADTabpanel  selectedADDetailTabpanel = null ;
				if(selectedADTabpanel.getCustomDetailPane() != null)
					selectedADDetailTabpanel = selectedADTabpanel.getCustomDetailPane().getSelectedADTabpanel();
			...
			if (columnOnClick != null && columnOnClick.trim().length() > 0) {
				...
				for(Object element : list) {
					if (element instanceof Div) {
						Div div = (Div) element;
						if (columnOnClick.equals(div.getAttribute(CustomGridTabRowRenderer.COLUMN_NAME_ATTR))) {
	 ...
	 * @param winPanel
	 */
	public void setADWindowPanel(CustomAbstractADWindowContent winPanel) {
		...
		if (isDetailPane()) {
			...
			while (parent != null) {
				if (parent instanceof CustomDetailPane.Tabpanel) {
					((CustomDetailPane.Tabpanel) parent).setPagingControl(gridFooter);
					break;
				}
				parent = parent.getParent();
			}
